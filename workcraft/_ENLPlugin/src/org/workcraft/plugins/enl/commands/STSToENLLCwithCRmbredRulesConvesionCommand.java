...
public class STSToENLLCwithCRmbredRulesConvesionCommand extends AbstractConversionCommand {

    @Override
    public String getDisplayName() {
        return "Net synthesis-[Check if ts can be synthesised to an ENL/LC-system and if so construct a solution satisfying criteria 1 & 2, and apply reduction rules]";
    }
    ...
    public ModelEntry convert(ModelEntry me) {
        ...
        if (TSmin.getTSmin(sts, 3)) {
            final STSToENLWayTowConverter converter = new STSToENLWayTowConverter(vsts, true);
...
//            System.out.println("Execution time is " + time + " milliseconds");
                    ...
                    DialogUtils.showInfo("The given ts can be synthesised w.r.t " + "\u224F" + "^{ts}_{min} "
                            + STS.getCoRe() + " to an ENL/LC-system with applying reduction rules.", "Result");
